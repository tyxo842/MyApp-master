...
public class GridViewMy extends GridView{
    ...
    public GridViewMy(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void startWobbleAnimation() {
        for (int i = 0; i < getChildCount(); i++) {
            View v = getChildAt(i);
            if (v != null && Boolean.TRUE != v.getTag(org.askerov.dynamicgrid.R.id.dgv_wobble_tag)) {
                if (i % 2 == 0)
                    animateWobble(v);
                else
                    animateWobbleInverse(v);
                v.setTag(org.askerov.dynamicgrid.R.id.dgv_wobble_tag, true);
            }
        }
    }
    ...
    private void stopWobble(boolean resetRotation) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (v != null) {
                ...
                v.setTag(org.askerov.dynamicgrid.R.id.dgv_wobble_tag, false);
            }
        }
    }
    ...
    public void init(Context context) {
        ...
        mOverlapIfSwitchStraightLine = getResources().getDimensionPixelSize(org.askerov.dynamicgrid.R.dimen.dgv_overlap_if_switch_straight_line);
    }
    ...
    private OnScrollListener mScrollListener = new OnScrollListener() {
        ...
        private void updateWobbleState(int visibleItemCount) {
            for (int i = 0; i < visibleItemCount; i++) {
                ...
                if (child != null) {
                    if (mMobileItemId != INVALID_ID && Boolean.TRUE != child.getTag(org.askerov.dynamicgrid.R.id.dgv_wobble_tag)) {
                        if (i % 2 == 0)
                            animateWobble(child);
                        else
                            animateWobbleInverse(child);
                        child.setTag(org.askerov.dynamicgrid.R.id.dgv_wobble_tag, true);
                    } else if (mMobileItemId == INVALID_ID && child.getRotation() != 0) {
                        child.setRotation(0);
                        child.setTag(org.askerov.dynamicgrid.R.id.dgv_wobble_tag, false);
                    }
                }
