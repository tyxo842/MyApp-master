...
import tyxo.mobilesafe.db.annotation.Table;
...
public class TableUtils {
	...
	private static void addColumns2Map(Class<?> entityType,
		...
		try {
			...
			HLog.e(TableUtils.class.getSimpleName(), e.getMessage());
	 ...
	 * key: entityType.name
	 */
	private static ConcurrentHashMap<String, tyxo.mobilesafe.db.table.Id> entityIdMap = new ConcurrentHashMap<>();

	/* package */
	static synchronized tyxo.mobilesafe.db.table.Id getId(
			Class<?> entityType) {
		if (Object.class.equals(entityType)) {
		...
		tyxo.mobilesafe.db.table.Id id = new tyxo.mobilesafe.db.table.Id(
	...
	private static String getPrimaryKeyFieldName(Class<?> entityType) {
		tyxo.mobilesafe.db.table.Id id = getId(entityType);
