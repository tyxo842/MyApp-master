...
public class Table {
    ...
    private Table(HDbManager db, Class<?> entityType) {
        this.db = db;
    ...
    public static synchronized Table get(HDbManager db, Class<?> entityType) {
    ...
    public static synchronized void remove(HDbManager db, Class<?> entityType) {
        String tableKey = db.getDaoConfig().getDbName() + "#" + entityType.getName();
        tableMap.remove(tableKey);
    }

    public static synchronized void remove(HDbManager db, String tableName) {
        if (tableMap.size() > 0) {
            String key = null;
