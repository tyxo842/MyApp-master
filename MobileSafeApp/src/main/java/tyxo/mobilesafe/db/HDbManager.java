...
import tyxo.mobilesafe.base.BaseApplication;
import tyxo.mobilesafe.db.exception.DbException;
import tyxo.mobilesafe.db.sqlite.CursorUtils;
import tyxo.mobilesafe.db.sqlite.DbModelSelector;
import tyxo.mobilesafe.db.sqlite.Selector;
import tyxo.mobilesafe.db.sqlite.SqlInfo;
import tyxo.mobilesafe.db.sqlite.SqlInfoBuilder;
import tyxo.mobilesafe.db.sqlite.WhereBuilder;
import tyxo.mobilesafe.db.table.DbModel;
import tyxo.mobilesafe.db.table.Id;
import tyxo.mobilesafe.db.table.Table;
import tyxo.mobilesafe.db.table.TableUtils;
import tyxo.mobilesafe.utils.IOUtils;
import tyxo.mobilesafe.utils.file.FileLocalManager;
...
public class HDbManager {
	 ...
	 * key: dbName
	 */
	private static HashMap<String, HDbManager> daoMap = new HashMap<String, HDbManager>();
	/**
	 * 默认数据库名
	 */
	public static final String DEFAULT_DATABASE_NAME = "hhdatabase.db";
	/**
	 * 默认数据库名
	 */
	private static String firstdbname = DEFAULT_DATABASE_NAME;
	...
	public static void setFirstdbname(String firstdbname) {
		HDbManager.firstdbname = firstdbname;
	}
	...
	private HDbManager(DaoConfig config) {
		if (config == null) {
	...
	private synchronized static HDbManager getInstance(DaoConfig daoConfig) {
		HDbManager dao = daoMap.get(daoConfig.getDbName());
		if (dao == null) {
			dao = new HDbManager(daoConfig);
			...
			if (oldVersion != 0) {
				...
				if (upgradeListener != null) {
					...
					try {
						...
						HLog.e(HDbManager.class.getSimpleName(), e.getMessage());
					}
				}
			}
	...
	public static HDbManager create(Context context) {
		DaoConfig config = new DaoConfig(context);
		return getInstance(config);
	}

	public static HDbManager create(Context context, String dbName) {
		DaoConfig config = new DaoConfig(context);
		config.setDbName(dbName);
		return getInstance(config);
	}

	public static HDbManager create(Context context, String dbDir, String dbName) {
		DaoConfig config = new DaoConfig(context);
	...
	public static HDbManager create(Context context, String dbDir,
	...
	public static HDbManager create(DaoConfig daoConfig) {
		return getInstance(daoConfig);
	}

	public HDbManager configDebug(boolean debug) {
		this.debug = debug;
		return this;
	}

	public HDbManager configAllowTransaction(boolean allowTransaction) {
	...
	public static class DaoConfig {
		...
		private String dbName = BaseApplication.getInstance().getPackageName()
				+ ".db";
		private int dbVersion = 1;
		private DbUpgradeListener dbUpgradeListener;
		private String dbDir = FileLocalManager.FILE_DATA_DIRECTORY;// 测试地址
		private boolean isDebug =false;// 是否调试中
		...
		public String getDbDir() {
			...
			if (isDebug) {// 测试地址
				if (dbDir == null)
					dbDir = FileLocalManager.FILE_DATA_DIRECTORY;
				HLog.i("zyw数据库文件路径",dbDir);
				
			} else {// 正式测试地址
				dbDir = BaseApplication.getInstance().getDatabasePath(dbName)
						.getParentFile().getPath();// 正式地址
			}
			return dbDir;
		}
	...
	public interface DbUpgradeListener {
		public void onUpgrade(HDbManager db, int oldVersion, int newVersion);
	}
	...
	private void debugSql(String sql) {
		if (debug) {
			HLog.d(getClass().getSimpleName(), "debugSql sql = " + sql);
		}
	}
